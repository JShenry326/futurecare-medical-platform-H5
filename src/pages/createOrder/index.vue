  ...
  <div class="container">
    ...
    <van-cell-group class="cell">
      <van-cell>
        ...
        <template #default
          ><div @click="showHospital = true">{{ from.hospital_name || '请选择就诊医院' }}<van-icon name="arrow" /></div>
        </template>
      </van-cell>
      <van-cell>
      ...
      <van-cell>
        ...
        <template #default>
          <div>
            <van-field class="text" input-align="right" v-model="from.receiveAddress" placeholder="请填写就诊人地址" />
          </div>
        </template>
      </van-cell>
      <van-cell>
        ...
        <template #default
          ><div>
            <van-field class="text" input-align="right" v-model="from.tel" placeholder="请填写您的联系电话" />
          </div>
        </template>
      </van-cell>
    </van-cell-group>
    ...
    <van-cell-group title="服务需求" class="cell">
      <van-field class="text" style="height: 100px" v-model="from.demand" placeholder="请简单描述您要就诊的科室.." />
    </van-cell-group>
    ...
    <van-popup v-model:show="showHospital" position="bottom">
      <van-picker :columns="hospitalColumns" @confirm="hospitalOnConfirm" @cancel="showHospital = false" />
    </van-popup>
    <van-popup v-model:show="showCompanion" position="bottom">
      <van-picker :columns="companionColumns" @confirm="companionOnConfirm" @cancel="showCompanion = false" />
    </van-popup>

    <van-popup v-model:show="showDate" position="bottom">
      <van-date-picker @confirm="dateOnConfirm" @cancel="showDate = false" v-model="currentDate" :min-date="minDate" title="选择日期" />
    </van-popup>
...
const companion_name = computed(() => {
  return createInfo?.value?.companion?.find(item => item.id === from.companion_id)?.name || '请选择陪诊师'
})
...
const sumbit = async () => {
  let arr = ['companion_id', 'demand', 'hospital_id', 'hospital_name', 'receiveAddress', 'starttime', 'tel']
  for (const i of arr) {
    if (!from[i]) {
      showNotify({ message: '请将表单填写完整' })
      return
    }
  }
